...
window.addEventListener("load", async () => {
    ...
    // 次數
    const counts = chatData.reduce((countIntent, { DisplayName }) => {
        if (DisplayName) {
            countIntent[DisplayName] = (countIntent[DisplayName] || 0) + 1;
        }
        return countIntent;
    }, {});

    const countsData = Object.entries(counts).map(([DisplayName, count]) => ({DisplayName, count}));

    const tableTopIntent = $("#tableTopIntent").DataTable({
        language: {
            url: "//cdn.datatables.net/plug-ins/1.10.25/i18n/Chinese-traditional.json"
        },
        order: [[1, 'desc']],
    ...
    function drawCharts(gender, minAge, maxAge) {
        ...
        const chatGenderArray = [['Gender', 'Count']];
        ...
        Object.entries(filteredChatMemberData).forEach(([ageGroup, genderData]) => {
            ...
            const toString = ageGroup.toString() + ' 歲（含）以上至 ' + toAge.toString() + ' 歲';
        ...
        chatGenderArray.push(['男性', totMale], ['女性', totFemale]);
        ...
        const chartChatGender = new google.visualization.PieChart(document.getElementById('piechart-chatGender'));
        chartChatGender.draw(google.visualization.arrayToDataTable(chatGenderArray), chartChatOptions);
    ...
    function drawMemberCharts() {
        ...
        memberGenderArray.push(['男性', memberMale], ['女性', memberFemale]);
    ...
    $("#minAge, #maxAge, #genderSelect").on("change", function() {
        ...
        $("#minAgeValue").text(minAge);
        $("#maxAgeValue").text(maxAge);
